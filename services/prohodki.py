...
def create_order(update, context):
    ...
    try:
        update.message.reply_text(f"Вы находитесь в процессе создания нового заказа.\n"
                                  ...
                                  f"Шаг 1/4. Введите вашу фамилию и имя на английском языке:\n")
...
def place(update, context):
    ...
    update.message.reply_text(
        ...
        f"бизнес-залах обычно можно найти на сайте аэропорта или по прилету.")
...
def lounge_name(update, context):
    ...
    update.message.reply_text(
        "Шаг 4/4. Ваша заявка сохранена, проходка зарезервирована и ожидает оплаты в течение 30 минут.\nПереведите "
        "оплату из расчёта 999 рублей за одного человека на карту.\nНомер карты: 5280413752652326\n\nВ подтверждение "
        "оплаты, пожалуйста, пришлите скриншот об оплате в данный чат боту.\n\nПосле оплаты в течение 15-30 минут вы "
        "получите подтверждение от оператора и QR-код. Его вместе с посадочным талоном вам нужно показать на входе в "
        "бизнес-зал. QR-код возврату и обмену не подлежит! Дети до 2-х лет проходят бесплатно. \nПриятного полета! :)")
...
def send_data_to_group(update, context):
    ...
    message_text2 = (f"Это заказ от юзера @{update.message.from_user.username}")
